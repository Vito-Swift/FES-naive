...
void fast_exhaustive_search(uint8_t **mqs, uint64_t var_num, uint64_t eq_num, uint64_t xvar_num, uint8_t *solution) {
    uint64_t* differ_counter = (uint64_t*) malloc(var_num * sizeof(uint64_t));
	...
	++chunk_size;
        ...
        uint64_t differ = __builtin_popcountll(func_eval);
	if (differ < mindiff) {
	    mindiff = differ;
	}
	if (chunk_size == 0x10) {
            chunk_size = 0;
            differ_counter[mindiff]++;
	    mindiff = 100;
	}
    }
    ...
    for (var_idx = 0; var_idx < var_num; ++var_idx)
        printf("\t\t[differ_counter] differ: %lld\tcount: %lld\n", var_idx, differ_counter[var_idx]);
